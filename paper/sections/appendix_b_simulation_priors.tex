...
\begin{table}[h]
\centering
\caption{Simulation prior distributions per experiment. The ``$\hat x$ vs threshold'' column is the simulated value's distance from its success threshold in prior-$\sigma$ units. The methylation track failure (row Exp~7 methyl) is engineered by setting $\mu_{\text{prior}}$ below the threshold; the simulated value is essentially at $\mu_{\text{prior}}$. The other simulated values are within $\pm 1\sigma$ of their prior means, none in the upper tail. $\simmark$}
\label{tab:appb-priors}
\begin{tabular}{llccccc}
\toprule
Experiment & Metric & $\mu_{\text{prior}}$ & $\sigma_{\text{prior}}$ & $\theta$ (success) & $\hat x$ (simulated) & $(\hat x{-}\theta)/\sigma_{\text{prior}}$ \\
\midrule
Exp~1 (baseline reproduction) & MAD vs published $\downarrow$ & $0.025$ & $0.010$ & $< 0.050$ & $0.023$ & $-2.7\sigma$ (pass) \\
Exp~2 (Phase-0 calibration) & probe-set fraction cleared & $0.60$ & $0.12$ & $\geq 0.30$ & $0.70$ & $+3.3\sigma$ above threshold \\
Exp~3 (phospho-PROTAC, HEADLINE) & $\auclift$ vs PTM-blind & $0.07$ & $0.02$ & $\geq 0.05$ & $0.087$ & $+1.85\sigma$ above threshold \\
Exp~4 (calibration ablation) & AUC drop when uncalibrated & $0.045$ & $0.010$ & $\geq 0.030$ & $0.041$ & $+1.1\sigma$ above threshold \\
Exp~5 (route ablation) & Pearson $r$ Boltz-2 vs MD & $0.70$ & $0.10$ & $\geq 0.70$ & $0.74$ & $+0.4\sigma$ above threshold \\
Exp~6 (scorer ablation) & $|\auclift|$ DeepTernary vs PROTAC-STAN & $0.030$ & $0.015$ & $< 0.050$ & $0.021$ & $-1.9\sigma$ (pass) \\
Exp~7 phospho track & $\auclift$ vs PTM-blind & $0.085$ & $0.020$ & $\geq 0.030$ & $0.087$ & $+2.85\sigma$ above threshold \\
Exp~7 mono-Ub track & $\auclift$ vs PTM-blind & $0.040$ & $0.020$ & $\geq 0.030$ (marginal $0.020$) & $0.034$ & $+0.2\sigma$ above marginal\\
Exp~7 methyl track & $\auclift$ vs PTM-blind & $\mathbf{0.020}$ & $0.020$ & $\mathbf{\geq 0.030}$ & $\mathbf{0.021}$ & $\mathbf{-0.45\sigma}$ (\textbf{FAILS by design}) \\
Exp~8 mutant-isoform & $|\Delta\auclift|$ vs phospho & $0.060$ & $0.020$ & $\geq 0.050$ & $0.058$ & $+0.4\sigma$ above threshold \\
\bottomrule
\end{tabular}
\end{table}
