...
`tools/discover.py` 采用轻量搜索漏斗：先构建 wiki interest profile，再从现有 API 抓候选，保留每个通道的 provenance，融合证据后排序，并用 anti-hub penalty 避免把泛化高引论文当成相关性。
...
- `wiki/papers/*.md`、`wiki/concepts/*.md`、`wiki/topics/*.md`、`wiki/methods/*.md` —— 标题与正文，用于 profile-aware 相关性打分
...
工具内部处理候选抓取、wiki dedup、证据融合、ranking，并写 checkpoint。始终传 `--wiki-root wiki`，否则已 ingest 论文会继续出现在 shortlist 中，浪费用户 review 时间。

venue 模式仍以 Paper Copilot 为候选来源：它只对完整的 venue/year 内存列表排序；配置了 S2/DeepXiv 时，可以用 profile search 作为加分信号，但只能按 arXiv ID 或精确标题匹配回 Paper Copilot 候选，不能把非 Paper Copilot 论文加入 venue shortlist。
...
- **始终对 wiki 做 dedup**：必须传 `--wiki-root wiki`，否则已 ingest 论文会污染 shortlist；精确标题 dedup 适用于所有 seed 模式，这是最常见的低质量失败模式。
- **流行度不是相关性**：citation count 只是次级 tie-breaker。候选需要具体的 wiki/profile 证据、anchor-specific 证据或 semantic-channel 证据；泛化高引 hub 论文应被降权。
...
- DeepXiv —— topic 模式下的 search 辅助通道，配置后也可做 venue profile boost（通过 `tools/fetch_deepxiv.py`，失败时优雅降级）
