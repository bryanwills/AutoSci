...
<!-- bio-C1（2026-05-11 minimal pilot merge）：bio-aware 标识符抽取。完整 C1 设计
     （PubMed/EuropePMC/CrossRef fetcher、`tools/extract_bio_ner.py`、JATS-XML 流水线）延后
     —— 那些计划的 Python 工具尚未实现。本最小切片让运行 /ingest 的 LLM 在 prompt 内做
     轻量级 NER，抽取 A3 minimal（papers/ 的 doi + pmid）与 A1 minimal（datasets/ 实体）
     真正用得上的生信标识符。 -->
### Step 2.5：生信标识符抽取（minimal C1）

对生信领域论文（正文、venue 或 DOI 前缀提示属于 biology / bioinformatics / chemistry /
clinical research —— 见 A8 的 bio domain 值：`structural-bio`、`chembio`、
`comp-drug-discovery`、`cancer-bio`、`systems-bio`、`bioinformatics`、`clinical-translation`）：

1. **填充 A3 minimal frontmatter 字段**，当值能从正文或已抓取的 metadata 源中读出时：
   - `doi` —— 主生信标识符（CrossRef 形态，`10.NNNN/...`）。通常印在论文头部 / 第一脚注。
   - `pmid` —— PubMed ID，仅当论文被 PubMed 索引。纯数字，≥ 7 位。

   不能自信抽取时留空——**不要编造**。A3 的其余字段（`biorxiv`、`pdb_ids`、`uniprot_ids`、
   `nct_ids`、`gene_symbols`、`species`）**延后到完整 C1 merge**，那时会加专用 fetcher 工具
   和结构化 NER 预扫。
2. **Dataset wikilink 升级**：在写 `## Method`、`## Experiment & Results` 或任何提及 dataset
   的正文章节之前，检查 `wiki/datasets/` 下是否存在 `title` 或 `aliases` 与该提及匹配
   （不区分大小写）的页面。匹配存在时，写 wikilink 形式 `[[{slug}]]` 而非纯文本。本最小
   pilot **不要**创建新的 `datasets/{slug}.md` 页面 —— dataset 页面创建交给 `/edit` 或完整
   C1 merge。

没有 arXiv ID 的生信论文 `arxiv:` 字段为空——A3 minimal 之后这是正常状态（`doi`/`pmid` 字段
才是它们的规范标识符）。完整 C1 会把 DOI / PMID / bioRxiv / PMC URL 加入 `/ingest` 接受的
源输入。
