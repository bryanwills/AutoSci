...
- `wiki/foundations/{slug}.md` —— 只允许 EDIT（追加 grounding / contribution 反向 section；foundation 不再是终端节点）
...
- `paper → method`：`proposes_method` / `applies_method` / `extends_method`，并写 `confidence`（反向 → method 的 `## Proposed by` / `## Applied by` / `## Extended by`）
- `paper → foundation`：`contributes_to_foundation`，并写 `confidence`（反向 → foundation 的 `## Contributed by papers`；foundation 不再是终端节点）
- `concept ↔ concept`：`shares_mechanism_with` / `shares_assumption_with` / `related_problem_formulation` / `contrasts_with_concept`（对称，写 `confidence`）—— 仅当原文明确表达该关系时
...
6. 新建 `wiki/concepts/{slug}.md` 时，若该 concept 明显归属一个或多个已有 topic（例如所有"self-improving coding agents"子题），设置 `parent_topics: [topic-a, topic-b]`（裸 slug —— `parent_topics` 是 `list_link` 类型）。反向是每个 topic 的 `key_concepts` frontmatter 列表，在同一 turn 内把该 concept slug 追加进去（或跑 `lint --fix`）。该 concept 依托的任何 foundation 也设 `grounded_in: [foundation-slug, ...]`（反向 → foundation 的 `## Grounds concepts`）。
7. 新建或更新 `wiki/methods/{slug}.md` 时，对该方法实现的任何 concept（通常是同篇论文引入的 concept）写入 `realizes_concepts: [[c1], [c2], ...]`，每个 concept 页的反向 `## Realized by` body 区在同一 turn 内追加。该方法依托的 foundation 设 `grounded_in: [foundation-slug, ...]`（反向 → foundation 的 `## Grounds methods`），明显归属某 topic 时设 `parent_topics`。
8. 通过 `add-edge`（带 `--confidence` 与 `--evidence`）记录论文与它讨论的每个 method 的关系：`proposes_method`（本文提出该方法）、`applies_method`（应用已有方法）或 `extends_method`（修改/扩展某方法）。method 页的反向 `## Proposed by` / `## Applied by` / `## Extended by` 区在同一 turn 内追加（或由 `lint --fix` 补）。
9. 当原文明确表达两个 concept 之间的关系时，通过 `add-edge`（带 `--confidence` 与 `--evidence`）加一条**对称** concept↔concept 边：`shares_mechanism_with`、`shares_assumption_with`、`related_problem_formulation` 或 `contrasts_with_concept`。不要臆造论文未陈述的关系。
...
- 每一条正向链接必须在同一 turn 内写入其反向链接 —— 这是 wiki 的双向链接不变量。foundations 不再是终端节点：`grounded_in` / `contributes_to_foundation` 链接的反向写入 foundation 的 `## Grounds concepts` / `## Grounds methods` / `## Contributed by papers` body 区(同一 turn 内追加,或跑 `lint --fix` 回填)。
