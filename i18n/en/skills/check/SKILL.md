...
`lint_bio.py` adds 6 checks the base linter cannot express: PDB ID format on `concepts.pdb_ids`, UniProt accession format on `concepts.uniprot_id`, dataset version cross-check against `datasets/{slug}.md::versions` (BOTH via `dataset_version_used` edge metadata.version (B3) AND via `experiments.reproducibility.dataset_versions[*]` (A8)), recognised-species check on `experiments.setup.species`, "MD assay → force_field required" on `experiments.setup`, and recognised-domain soft check on `<entity>.domain` across all entity types (A4 — encourages convergence on canonical slugs: structural-bio / chembio / comp-drug-discovery / cancer-bio / systems-bio / bioinformatics / clinical-translation / nlp / cv / ml-systems / robotics / ml-for-science / rl / theory / other). Merge its JSON output into the same report (same severity convention 🔴/🟡/🔵; `bio-*` category prefix distinguishes bio findings from base findings). Skip silently if no bio fields are present anywhere — non-bio wikis pay zero cost.
