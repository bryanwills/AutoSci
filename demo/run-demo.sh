...
#                      去重 + 打信号 + 构造上下文
#   2) recommend-llm — DeepSeek v4-flash ranks candidates (requires DEEPSEEK_API_KEY)
#                      DeepSeek v4-flash 排序（需要 DEEPSEEK_API_KEY）
...
if [ -n "${DEEPSEEK_API_KEY:-}" ] || grep -qs '^DEEPSEEK_API_KEY=' .env ~/.env 2>/dev/null; then
  ...
  echo "[2/3] DEEPSEEK_API_KEY not set — skipping LLM ranking; finalize will use tool signals only / 未设 DEEPSEEK_API_KEY，跳过 LLM 排序，finalize 退化为 tool-only"
  DECISIONS_ARG=()
fi
