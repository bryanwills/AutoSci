...
source_papers: ["[[drug-design-targeting-active-posttranslational-modification]]"]
evidence:
  - source: "[[drug-design-targeting-active-posttranslational-modification]]"
    type: supports
    strength: moderate
    detail: "Review enumerates clinical wins where a PTM-isoform target gave selectivity unattainable on the wild-type protein: asciminib (ABL001) at the Bcr-Abl myristoyl pocket (Phase III positive in CML and Ph+ ALL); tazemetostat as a first-in-class EZH2 inhibitor (FDA-approved 2020 for epithelioid sarcoma); HDAC inhibitors approved for T-cell lymphomas; thalidomide-class CRBN binders for multiple myeloma; and a covalent RhoA inhibitor identified by the authors using the PTMI-DD framework on a target widely considered undruggable. Statistical analysis of modified vs. unmodified PDB chains shows phosphorylation and N-glycosylation produce significant structural changes that can yield isoform-specific surfaces."
conditions: "Holds when (i) a disease-relevant PTM-isoform of the target is well characterised, (ii) the PTM creates a chemically or conformationally distinct pocket / PPI interface / E3-recruitable surface, and (iii) cellular target engagement can be measured for the isoform vs. the wild-type. The reviewed cases are skewed toward kinases, epigenetic writers, and CRBN-amenable degraders; generality across the proteome is not yet established."
...
- Restricted to small-molecule and PROTAC modalities; antibody and biologics evidence is mentioned but not central.
