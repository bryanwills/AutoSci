...
evidence:
  - source: highly-accurate-protein-structure-prediction-alphafold
    type: supports
    strength: strong
    detail: "Figure 5a shows that AlphaFold2's lDDT-Calpha drops sharply when the median per-residue Neff falls below ~30, with a threshold-like transition. Improvements above Neff ~100 yield small additional gains. The pattern holds across both the high-template-coverage and low-template-coverage subgroups of the post-cutoff PDB set, indicating the dependence is on MSA-derived covariation signal rather than template availability."
conditions: "Statement applies to AlphaFold-family architectures that consume an MSA as a primary input and rely on covariation signal to triangulate residue-pair distances. May not transfer directly to single-sequence or protein-language-model-based predictors that do not use an MSA at inference time."
...
- Holds for predictors whose inference pipeline consumes an MSA. Single-sequence predictors built on protein language models may exhibit a different scaling law (open question).
