...
## 2026-05-12 —— C3 minimal pilot merge：`/ideate` banlist 加 scope 字段

**范围**：`/ideate` banlist 当前是全局的 —— 失败 idea 的 `failure_reason` 在任何方向都触发屏蔽。
但 `ptm-site-disorder-predictor` 的饱和理由（SAPP / PhosAF / GraphPhos / AstraPTM2 / DeepPCT
/ MTPrompt-PTM）仅适用于人/鼠 phospho 高数据子空间;植物 phospho、微生物 PTM、跨物种低数据迁移
都未饱和。C3 minimal 给 ideas/ 加 `scope` 对象（species / disease_area / data_regime）,
banlist 仅在 scope overlap 时命中。
**状态**：**C3 minimal 合并**。完整 C3（scope 字段更细的子空间编码,如 ptm_type / cell_type /
modality;基于 graph 边自动推断 scope；scope-aware banlist 命中度量）延后。

### 改动位置

- `runtime/schema/entities.yaml`：ideas/ 加 `scope` 对象字段（type: object,3 个可选子字段
  species / disease_area / data_regime）。加法变更 —— 空 / 缺失视为 "universal"（legacy 行为）。
- `i18n/{en,zh}/skills/ideate/SKILL.md` + `.claude/skills/ideate/SKILL.md`：
  - Phase 1 Load wiki: 加载 banlist 时同时读 scope
  - Phase 2 Review LLM input: banlist 段显式列出每条 scope
  - Phase 3 Filter decision: 加 scope overlap 规则（任一方 scope 空 → universal match;
    否则 species + disease_area 交集非空 + data_regime 一致 → overlap;不重叠时记 IDEA_REPORT
    "scope-distinct prior",**不**淘汰）
  - Phase 5 Write top ideas: frontmatter 模板加 scope 块
  - Phase 5 Write eliminated ideas: scope 在 failed 上**必填**（即便全空也要显式 —— 让 universal block 是有意决策）
- `docs/runtime-page-templates.{en,zh}.md`: ideas 模板加 scope 子块。
- **Live backfill 2 个 failed ideas**：
  - `wiki/ideas/ptm-site-disorder-predictor.md`：scope.species=[human, mouse]、
    data_regime=high_data —— 饱和工具集 specifically 是人/鼠 phospho 高数据;植物 / 微生物 /
    低数据 PTM 不在屏蔽内。
  - `wiki/ideas/chirality-aware-af3-diffusion.md`：scope 全空 = universal block —— 失败原因
    是 AF3 license 而非子空间饱和,任何方向的 AF3 fine-tune idea 都应命中。
...
| `diff -q i18n/en/skills/ideate/SKILL.md .claude/skills/ideate/SKILL.md` | 一致 |
| `grep -c "scope:" wiki/ideas/*.md` | ≥ 2（2 个 failed idea 回填） |
| 活动 SKILL.md 行数 | 414（升自 403） |

Section C 状态：7/9 → 8/9。仅余 C7（/exp-run 目录布局）。

---

## 2026-05-12 —— C7 minimal pilot merge：`/exp-run` 按 setup type 路由 4 种布局

**范围**：`/exp-run` Phase 1 Step 3 此前一律写 `train.py + config.yaml + run.sh + requirements.txt`
ML 流水线形态。MD 自然的文件是 `mdrun.sh + system.{gro,pdb,top} + mdp/*.mdp`,湿实验是
`protocol.md + analysis.ipynb + data/`,docking 又不同。C7 minimal 按 A5-full 的
`setup.in_silico_or_wet` + `setup.assay_type` 路由到 4 种布局。
**状态**：**C7 minimal 以纯 prompt 形态合并**。完整 C7（`skills/exp-run/references/templates/{ml,md,
wet-lab,docking}/` 具体模板文件）延后 —— minimal pilot 用文件清单 + agent scaffold。
**Section C 收尾**：本次后 Section C 状态从 8/9 升至 **9/9 完结**。

### 改动位置

- `i18n/{en,zh}/skills/exp-run/SKILL.md` + `.claude/skills/exp-run/SKILL.md`：
  - **Writes** 列表展开为 4 种布局
  - **Phase 1 Step 1** 增读 A5-full setup 字段（in_silico_or_wet / assay_type / force_field /
    solvent_model / simulation_length / species / cell_line / weight_version /
    random_seed_protocol）
  - **Phase 1 Step 3** 加 "Setup-type routing" 块：if 树（wet_lab → wet-lab;MD pattern → MD;
    docking → docking;else → ML）+ 4 种布局的逐文件描述,每个文件说明它消费哪些 A5-full 字段。
    "mixed" 默认走 in-silico,非平凡湿实验时 scaffold 并列 `wet-lab/` 子目录。
  - **Local + Remote launch** 参数化 `ENTRY` 变量（`run.sh` / `mdrun.sh` / `dock.sh`）。
    Wet-lab 不自动启动 —— 仅 scaffold + 提示用户手动执行,`--collect` 再进入。
  - 全部 4 布局结果保持 `results/{slug}/seed_{N}.json` shape,下游 `/exp-eval` + `/paper-draft`
    不分支。
...
## 2026-05-12 —— D1 + D2 minimal pilot merge：约定冲突收尾

**范围**：Section D 两项约定冲突。D1 —— bio "Phase" 与 skill "Stage" 编号冲突;D2 —— `concepts.maturity` 的 CS feature-flag 谱系与 bio 证据强度谱系不匹配。
**状态**：**D1 + D2 minimal 合并**。D1 走 disambiguation note（不重命名 —— 40+ 个 Stage 引用跨 10 页面太重）;D2 扩 enum 加 bio 5 项。完整 D1（rename Stage → Block-X 全部 wiki 内容）+ 完整 D2（lint 检测同一 concept 内混用 scale）延后。

### 改动位置

- `i18n/{en,zh}/skills/exp-design/SKILL.md` + `.claude/skills/exp-design/SKILL.md`:
  Step 4 Build Run Order 开头加 "Vocabulary disambiguation" 块 —— 显式声明 skill 用 "Stage 0..4" 表运行顺序、**绝不**用 "Phase";"Phase" 留给用户领域含义（Phase 0 临床前 / Phase 1-3 临床试验）。解析 idea 含 "Phase-N" 时视为领域词汇,不静默重映射。两词同存例:"Stage 2b runs the Phase-0 noise-floor calibration sub-protocol"。
- `runtime/schema/entities.yaml`：`concepts.maturity` enum 从 4 项扩到 9 项 —— CS 4 项保留（stable / active / emerging / deprecated）+ bio 5 项加入（consensus / well-supported / contested / hypothesis / falsified）。`datasets.maturity` 保持 CS 4 项不变（数据集是操作性工具,bio 证据强度谱不适用）。
- `docs/runtime-page-templates.{en,zh}.md`：concepts 模板 maturity 注释加 bio 5 项 + 说明每 concept 选一个 scale 不混用。
...
| `python tools/lint_bio.py` | 0 🔴 / 0 🟡 / 0 🔵 |
| `diff -q i18n/en/skills/exp-design/SKILL.md .claude/skills/exp-design/SKILL.md` | 一致 |
| 现有 concept 页面 maturity 值 | 全部 CS scale,新 enum 向后兼容 |

Section D backlog 子项（D1 + D2）已 minimal 合并。
